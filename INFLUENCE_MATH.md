...
```
    ...
    Content
       ...
       v
    +------------------+
    | ATTENTION SYSTEM  |    Intake mode, media type, primal triggers
    | (Impression)      |    -> consumed_prob, interact_prob, attention_cost
    +------------------+
       |
       v
    +------------------+
    | IDENTITY SPACE    |    Agent demographics -> dimensional coordinates
    | (Similarity)      |    -> exp(-dist/decay) per dimension, weighted sum
    +------------------+
       |
       v
    +------------------+
    | INFLUENCE WEIGHT  |    similarity -> homophily boost [0.3x .. 1.6x]
    | (Trust Gate)      |    * status_boost * base_trust
    +------------------+
       |
       v
    +===========================+
    ...
    |   (11 Steps)              |
    |                           |
    |  1. Trust gate            |    trust^gamma (superlinear)
    |  2. Bounded confidence    |    reject if |diff| > tau
    |  3. Habituation           |    1/(1 + alpha*n)
    |  4. Base influence        |    trust * cred * primal * proximity
    |  5. Identity defense      |    backfire / confirm / oppose
    |  6. Evidence accumulation |    E = lambda*E + w*s, gate at theta
    |  7. Inertia & momentum    |    v = rho*v + eta*E
    |  8. Critical velocity     |    eta_eff if |v| > v_thresh
    |  9. Rebound force         |    -k*(stance - core_value)
    | 10. Stance update         |    stance += eta_eff*v + rebound
    | 11. Confidence update     |    +/- based on confirm/oppose
    |                           |
    +===========================+
       |
       v
    +------------------+
    | DAILY CONSOLIDATE |    Dream: drift, salience, knowledge
    | (Identity Update) |    Identity: rigidity, ingroup labels
    +------------------+
```
...
$$\vec{p}_{\text{pol}} = \begin{pmatrix} x \\ -0.8x \\ 0 \\ 0.5x \\ 0.4x \end{pmatrix} \quad \text{where } x = \text{political\_ideology} \in [-1, +1]$$

Or from full `PoliticalIdentity`:

$$\vec{p}_{\text{pol}} = \begin{pmatrix} \text{economic\_left\_right} \\ \text{social\_progressive\_traditional} \\ \text{libertarian\_authoritarian} \\ \text{cosmopolitan\_nationalist} \\ \text{secular\_religious} \end{pmatrix}$$
...
Weights auto-normalize: $w_d' = w_d / \sum_d w_d$.
...
$$S = \text{IdentitySpace::compute\_similarity}(a, b)$$
...
$$h(S) = \begin{cases} 0.3 + S & \text{if } S < 0.3 \quad \text{(out-group attenuation)} \\ 1.0 & \text{if } 0.3 \le S \le 0.7 \quad \text{(neutral)} \\ 1.0 + 2(S - 0.7) & \text{if } S > 0.7 \quad \text{(in-group amplification)} \end{cases}$$

### 3.3 Status Boost

$$B_{\text{status}} = 1.0 + 0.2 \cdot \mathbb{1}[\text{verified}] + 0.3 \cdot \mathbb{1}[\text{influencer}]$$
...
$$P_{\text{consume}} = \text{clamp}_{[0,1]}\!\big(\text{base}_{\text{consume}}(m) \cdot \text{mult}_{\text{consume}}(i)\big)$$

### 4.4 Primal Activation

$$A_{\text{primal}} = \text{clamp}_{[0,1]}\!\left(\text{base}_{\text{primal}}(m) + \sum_t w(t) + 0.4 \cdot \text{intensity}\right)$$

where trigger weights $w(t)$ are:
...
$$t_g = \text{trust}^{\gamma} \qquad \gamma = 2.0$$
...
**Effect**: An agent with trust 0.3 has effective trust $0.3^2 = 0.09$. Trust 0.8 gives $0.64$. Low trust is nearly useless.

### Step 2: Bounded Confidence

$$\text{if } |s_{\text{signal}} - b_t| > \tau \implies \text{REJECT} \qquad \tau = 1.5$$

Signals too far from current stance are ignored entirely.

### Step 3: Habituation

$$w_h = \frac{1}{1 + \alpha \cdot n_{\text{exposures}}} \qquad \alpha = 0.05$$
...
Repeated exposures to the same topic yield diminishing returns.

### Step 4: Base Influence Multiplier

$$M_{\text{base}} = t_e \cdot M_{\text{cred}} \cdot M_{\text{primal}} \cdot M_{\text{prox}} \cdot M_{\text{scroll}} \cdot w_h$$

where:

$$t_e = t_g \cdot (1 + 0.15 \cdot \text{proximity} + 0.25 \cdot \text{social\_proof}) \cdot \begin{cases} 1.2 & \text{self-source} \\ 1.0 & \text{otherwise} \end{cases}$$
...
$$M_{\text{scroll}} = \begin{cases} 0.35 & \text{SCROLL intake} \\ 1.0 & \text{otherwise} \end{cases}$$

### Step 5: Identity Defense and Confirmation Bias

Let $\Delta s = s_{\text{signal}} - b_t$ (stance difference).

$$\Delta s' = \begin{cases} -0.4 \cdot \Delta s & \text{if identity\_threat} > 0.5 \text{ AND } |\Delta s| > 1.0 \quad \textbf{(backfire)} \\ 1.1 \cdot \Delta s & \text{if } \Delta s \cdot b_t > 0 \quad \textbf{(confirmation boost)} \\ 0.6 \cdot (1 - r_{\text{rig}}) \cdot \Delta s & \text{if } |\Delta s| > 1.0 \quad \textbf{(opposed, openness-gated)} \\ \Delta s & \text{otherwise} \end{cases}$$

```
    IDENTITY DEFENSE LOGIC
    ======================
              ...
              YES         NO         YES          NO
               |           |          |            |
           BACKFIRE     normal    CONFIRM       Opposed?
          ds' = -0.4*ds          ds' = 1.1*ds  (|ds|>1)
                                               /     \
                                             YES      NO
                                              |        |
                                          REDUCE    normal
...
$$E_t = \lambda \cdot E_{t-1} + w \cdot \Delta s' \qquad \lambda = 0.90, \quad w = 0.1 \cdot M_{\text{base}}$$

$$\text{if } |E_t| < \theta \implies \textbf{NO STANCE UPDATE} \qquad \theta = 0.5$$
...
This is the primary reason single exposures rarely change beliefs. Evidence must accumulate across multiple aligned exposures before the threshold is crossed.

### Step 7: Inertia and Momentum

$$v_{t+1} = \rho \cdot v_t + \eta \cdot E_t \qquad \rho = 0.85, \quad \eta = 0.10$$

Momentum persists across ticks, creating belief trajectory.

### Step 8: Critical Velocity (Nonlinear Gain)

$$\eta_{\text{eff}} = \eta \cdot \big(1 + \kappa \cdot \sigma(|v_t| - v_0)\big) \qquad \kappa = 2.0, \quad v_0 = 0.1$$
...
Once an agent is already moving in a direction, additional aligned influence becomes easier. This models the "tipping point" where accumulated evidence creates a cascade.

### Step 9: Rebound Force (Damped Spring)

$$F_{\text{rebound}} = -k \cdot (b_t - b_0) \qquad k = 0.05$$
...
```
    Force
      ^
      |     * <-- pull right (stance < core)
      |    *
      |   *
    0 +--*--*--*--*--*--*--*--> stance - core_value
      |               *
      |                *
      |                 * <-- pull left (stance > core)
      v
```

### Step 10: Final Stance Update

$$b_{t+1} = \text{clamp}_{[-1,1]}\!\big(b_t + \eta_{\text{eff}} \cdot v_{t+1} + F_{\text{rebound}}\big)$$

### Step 11: Confidence Update

$$\Delta c = \begin{cases} +0.04 \cdot t_e \cdot M_{\text{base}} & \text{confirming} \\ -0.02 \cdot t_e \cdot M_{\text{base}} & \text{opposing} \end{cases} + \begin{cases} +0.05 & \text{self-source} \\ 0 & \text{otherwise} \end{cases}$$

$$\Delta c = \text{clamp}(\Delta c, -0.2, +0.2)$$

After update: $E_t \leftarrow 0.5 \cdot E_t$ (partial evidence reset).
...
$$\Delta b = \Delta s \cdot 0.10 \cdot t_e \cdot M_{\text{prox}} \cdot M_{\text{cred}} \cdot M_{\text{primal}} \cdot M_{\text{scroll}}$$
...
$$r_{t+1} = \text{clamp}_{[0.05, 0.95]}\!\big(r_t + 0.05 \cdot (\bar{T} - 0.25)\big)$$

High average identity threat increases rigidity; low threat decreases it.

### 7.3 Identity Vector Drift

For each identity dimension $j$:

$$\text{push}_j = \frac{\sum_i w_i \cdot s_i \cdot \mathbb{1}[h(t_i) = j]}{\sum_i w_i}$$

$$\vec{I}_{j,t+1} = \text{clamp}_{[-1,1]}\!\big(\vec{I}_{j,t} + 0.03 \cdot (1 - r_t) \cdot \text{push}_j\big)$$
...
$$\bar{s}_t = \frac{\sum_i s_i}{c} \qquad \text{(mean stance signal for topic)}$$
...
$$\bar{\chi} = \sum_k \omega_k \cdot \chi_k \qquad \bar{r} = \sum_k \omega_k \cdot r_k$$
...
$$\mu_{t+1} = \text{clamp}_{[-1,1]}\!\big(\mu_t + 0.10 \cdot (1 - \bar{r}) \cdot v_{t+1} + F\big)$$
